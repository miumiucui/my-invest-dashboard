...
def get_all_data():
    # 获取 550 天数据，确保 MA200 计算稳定
    tickers = ["BTC-USD", "TSLA", "AAPL", "AMZN"]
    prices = yf.download(tickers, period="550d", auto_adjust=True, progress=False)["Close"]
    btc_vol = yf.download("BTC-USD", period="550d", auto_adjust=False, progress=False)["Volume"]
    prices = prices.dropna(how="all")
    btc_vol = btc_vol.dropna()
    return prices, btc_vol
...
try:
    ...
    c1.metric("AAPL 实时价格", f"${aapl_last:.2f}", format_pct(aapl_change))
    c2.metric("AMZN 实时价格", f"${amzn_last:.2f}", format_pct(amzn_change))
    c3.metric("TSLA 实时价格", f"${tsla_last:.2f}", format_pct(tsla_change))
    c4.metric("BTC 实时价格", f"${btc_last:,.0f}", format_pct(btc_change))
    st.divider()
